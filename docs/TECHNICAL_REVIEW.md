...
| Trei selectori + fallback legacy care ocolește `is_retryable()` + `SessionAccount` fără TTL | Înaltă | ⏳ OPEN (parțial: fallback legacy at-most-once fixat în `c0d1e2f`; `InferFailed.retryable` respectat; selectorii unificați + SessionAccount TTL rămân) |
