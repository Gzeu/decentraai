...
| Trei selectori + fallback legacy care ocolește `is_retryable()` + `SessionAccount` fără TTL | Înaltă | ⏳ OPEN (parțial: fallback legacy at-most-once fixat în `b1efa8a`; `InferFailed.retryable` respectat; SessionAccount TTL fixat în `50c1783`; rămâne unificarea celor trei selectori) |
