...
> **Provenance & status (hardening sprint, 2026-09-06):** adopted from PR #83
> (`fix/code-review-findings`, closed superseded — branch predated current
> fmt toolchain). Findings F1–F20 below are filed VERBATIM against the
> commits they name. tracked status: **F1 FIXED** (typed `tx_status`,
> `crates/runtime/src/settlement_tx.rs`) · **F2 FIXED** (resubmit cap,
> `OnChainProof::resubmit_count`) · F3–F20 OPEN as filed (triage next).
