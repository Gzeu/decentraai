...
use decentraai_protocol::{InferRequest, InferResponse, WorkerAnnouncement, TaskPlacement};
...
impl FallbackHandler {
    ...
    pub async fn get_excluded_workers(&self, request_id: Uuid) -> Vec<PeerId> {
        self.excluded_workers.lock().await
            .get(&request_id)
            .cloned()
            .unwrap_or_default()
    }
    ...
    pub async fn route_with_fallback(
        ...
        match router.send_request(p2p_node, current_request.clone(), primary_worker.clone()).await {
            Ok(response) => {
                info!(
            ...
            Err(e) => {
                ...
                // Record the failure
                let can_retry = self.record_failure(request_id, primary_worker.selected_worker).await;
                if !can_retry {
        ...
        for worker in &workers {
            ...
            match router.send_request(p2p_node, current_request.clone(), placement).await {
                Ok(response) => {
                    info!(
                        request_id = %request_id,
                        worker_peer_id = %worker.peer_id,
    ...
    pub async fn wait_backoff(&self, attempt: u32, base_ms: u64) {
        ...
        debug!(attempt, backoff_ms = backoff.as_millis(), "waiting before retry");
...
mod tests {
    ...
    fn test_calculate_backoff() {
        // Base of 100ms
        assert_eq!(FallbackHandler::calculate_backoff(0, 100), Duration::from_millis(100));
        assert_eq!(FallbackHandler::calculate_backoff(1, 100), Duration::from_millis(200));
        assert_eq!(FallbackHandler::calculate_backoff(2, 100), Duration::from_millis(400));
        assert_eq!(FallbackHandler::calculate_backoff(3, 100), Duration::from_millis(800));
    }
}
