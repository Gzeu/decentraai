...
/// Executes a delegated task and returns the output JSON value.
///
/// This is the seam to the real engine: the runtime calls this with the
/// task's inputs; the callback runs the model/tool and returns an output
/// `serde_json::Value`. Held as `Arc<dyn Fn>` so it is `Send + Sync`.
pub type AgentExecutor = dyn for<'a, 'b> Fn(
    &'a AgentTask,
    &'b serde_json::Value,
) -> Result<serde_json::Value> + Send + Sync;
...
impl AgentRuntime {
    ...
    pub fn with_executor<F>(&mut self, executor: F) -> &mut Self
    where
        F: for<'a, 'b> Fn(&'a AgentTask, &'b serde_json::Value) -> Result<serde_json::Value>
            + Send
            + Sync
            + 'static,
    {
    ...
    pub async fn process_one(&self) -> Result<Option<ExecutedMessage>> {
        ...
        match executor(&task, &inputs) {
            Ok(output) => {
                ...
                // Reply to the delegating peer. A transport failure is logged;
                // the orchestrator surfaces the stage failure via its timeout.
                self.messenger
...
mod tests {
    ...
    async fn can_execute_reflects_executor() {
        ...
        runtime.with_executor(|_t, _i| Ok(serde_json::Value::Null));
        assert!(runtime.can_execute());
    }
}
