...
use crate::{
    AgentId, ProposalId, ReputationEvent, ReputationEventType, SocietyError, TaskId, Tick,
    reputation::SocialReputation,
    state::{
        ContributionRecord, RewardDistribution, ShareBasis, SocialRelationship, TaskOutcome,
        TaskOutcomeStatus,
    },
};
use decentraai_agent_hub::{Bid, HubEvent, HubTask, Proposal, ProposalStatus, TaskStatus, Team};
...
impl SocietyRules {
    ...
    fn evaluate_propose_opportunities(
        &self,
        ctx: &DecisionContext,
        task: &HubTask,
    ) -> Vec<DecisionHint> {
        ...
        for bid in bidders {
            ...
            let combined =
                trust * (1.0 - self.reputation_weight) + rep_score * self.reputation_weight;

            if combined >= self.trust_threshold_collaborate {
                // Make counter-offer above bid but below reward
                let min_offer =
                    (bid.price as f32 * (1.0 + self.counter_offer_min_improvement)) as u64;
                ...
                if offer_price > bid.price {
                    hints.push(DecisionHint {
                        ...
                        rationale: format!(
                            "Bidder {} has trust {:.2}, rep {:.2}; counter-offer {} > {}",
                ...
                hints.push(DecisionHint {
                    action: SuggestedAction::Refuse {
                        reason: format!(
                            "Low trust ({:.2}) + rep ({:.2}) for {}",
                            trust, rep_score, bid.bidder
                        ),
                    },
        ...
        if let Some(task) = task {
            if task.status == TaskStatus::Assigned || task.status == TaskStatus::InProgress {
                ...
                if is_issuer || is_member {
                    return vec![DecisionHint {
                        action: SuggestedAction::ExecuteTask {
                            task_id: task.id.clone(),
                        },
            ...
            if trust > self.trust_threshold_refuse && rep_score > -0.5 {
                hints.push(DecisionHint {
                    action: SuggestedAction::PlaceBid {
                        ...
                        rationale: format!(
                            "Task {} reward {}, bid {}",
                            task.id, task.reward, bid_price
                        ),
                    },
                    rationale: format!(
                        "Open task {} reward {}, trust {:.2} with issuer {}",
                        task.id, task.reward, trust, task.issuer
                    ),
    ...
    fn evaluate_task_publishing(&self, ctx: &DecisionContext) -> Vec<DecisionHint> {
        ...
        if ctx.resources.quota_available > 200
            && ctx.hub.my_tasks.len() < 3
            && ctx.hub.total_tasks < 20
        {
            ...
            return vec![DecisionHint {
                ...
                rationale: format!(
                    "Have quota {}, few tasks, publishing new",
                    ctx.resources.quota_available
                ),
                confidence: 0.4,
                alternatives: vec![AlternativeAction {
                    action: SuggestedAction::Wait,
    ...
    pub fn validate_counter_offer(
        ...
        if counter_price <= original_price {
            return Err(SocietyError::InvalidCounterOffer(
                "Counter must exceed original".to_string(),
            ));
        }
        if counter_price > task_reward {
            return Err(SocietyError::InvalidCounterOffer(
                "Counter exceeds task reward".to_string(),
            ));
        }
        ...
        if improvement < self.counter_offer_min_improvement {
            return Err(SocietyError::InvalidCounterOffer(format!(
                "Counter improvement {:.1}% below minimum {:.1}%",
                improvement * 100.0,
                self.counter_offer_min_improvement * 100.0
            )));
        }
        Ok(())
    }
    ...
    pub fn validate_refusal(&self, trust: f32, reason: &str) -> Result<(), SocietyError> {
        if trust > self.trust_threshold_refuse && reason.is_empty() {
            return Err(SocietyError::RefusalNotAllowed(
                "Refusal requires reason when trust is neutral/positive".to_string(),
            ));
        }
        Ok(())
    }
    ...
    pub fn calculate_distribution(
        ...
        for contrib in contributions {
            ...
            distributions.push(RewardDistribution {
                ...
                amount,
                share_basis: if contrib.verified_contribution.is_some() {
                    ShareBasis::Verified
                } else {
                    ShareBasis::Planned
                },
