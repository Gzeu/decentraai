...
pub fn training_candidates(entries: &[MemoryEntry]) -> Vec<TrainingCandidate> {
    ...
    for e in entries {
        ...
        {
            failures
                .entry(e.meta.subject_key.as_str())
                .or_insert(&e.content);
        }
    }
    entries
        ...
        .map(|e| TrainingCandidate {
            ...
            paired_failure: if e.meta.kind == KnowledgeKind::Solution
                && !e.meta.subject_key.is_empty()
            {
                failures
                    .get(e.meta.subject_key.as_str())
                    .map(|f| f.chars().take(1024).collect())
            } else {
                None
            },
        })
...
mod tests {
    ...
    fn evidenced_entry(
        ...
        let mut e =
            MemoryEntry::new(id, "learnings", "researcher", "node-1", content).with_kind(kind);
    ...
    fn only_verified_and_evidenced_generalizations_export() {
        ...
        assert_eq!(
            ids,
            vec!["ok", "sol"],
            "only verified+evidenced generalizations"
        );
    ...
    fn failure_pairs_with_its_verified_solution_only() {
        let entries = vec![
            ...
            verified_with_subject(
                "f1",
                KnowledgeKind::Failure,
                MemoryStatus::Verified,
                Some("aud-f"),
                "llama-server OOM at 8k ctx",
                "q:oom",
            ),
        ...
        assert_eq!(
            by_id("s1").paired_failure.as_deref(),
            Some("llama-server OOM at 8k ctx")
        );
        assert_eq!(
            by_id("s2").paired_failure,
            None,
            "unverified failure poisons nothing"
        );
        assert_eq!(
            by_id("s3").paired_failure,
            None,
            "obsolete failure does not pair"
        );
    }
}
