...
pub async fn resolve_provider_model(
    ...
    // forward them verbatim to the provider backend. Backends that support
    // function calling (or emulate it, e.g. the Perchance bridge) answer with
    // `tool_calls`; without tools the request shape is unchanged.
