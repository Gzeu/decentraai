...
async fn main() -> anyhow::Result<()> {
    ...
    println!("This demo mirrors `cargo test external_agent_gateway_three_agent_economy -- --nocapture`");
    ...
    if let Ok(endpoint) = std::env::var("DECENTRAAI_ENDPOINT") {
        ...
        println!("→ Live mode: endpoint={endpoint}, key={}...", &key[..8.min(key.len())]);
...
async fn live_demo(endpoint: String, key: String) -> anyhow::Result<()> {
    ...
    let r: serde_json::Value = client
        ...
        .json()
        .await?;
    println!("discover_capabilities → {}", serde_json::to_string_pretty(&r)?);
    println!("\nNext steps for your agent (copy-paste):");
    println!("  curl -X POST $ENDPOINT/mcp -H \"Authorization: Bearer $KEY\" -d '{{\"jsonrpc\":\"2.0\",\"id\":2,\"method\":\"tools/call\",\"params\":{{\"name\":\"hub_state\",\"arguments\":{{}}}}}}'");
    println!("  ... then hub_publish_task / hub_place_bid / hub_propose / hub_form_team / hub_execute");
    Ok(())
}
...
async fn ephemeral_demo() -> anyhow::Result<()> {
    ...
    let manager = {
        ...
        let app = axum::Router::new()
            .route("/v1/models", axum::routing::get(|| async { "{\"object\":\"list\",\"data\":[]}" }))
            .route("/v1/chat/completions", axum::routing::post(|| async { "{\"choices\":[]}" }));
        let listener2 = tokio::net::TcpListener::bind("127.0.0.1:0").await.unwrap();
        let _ = listener2;
        tokio::spawn(async move { axum::serve(listener, app).await.unwrap(); });
        Arc::new(Mutex::new(ServeManager::new(srv, Duration::from_secs(3600))))
    };
    let ledger = Arc::new(std::sync::Mutex::new(QuotaLedger::new(ContributionPolicy::default())));
    {
        let mut l = ledger.lock().unwrap();
        for acc in ["agent-a","agent-b","agent-c"] { l.credit(&acc.to_string(), "seed", Some(5000), None); }
    }
    let mut state = ApiState::new(
        ...
        None, None, InferenceQueue::new(5, Duration::from_secs(5)), None, None,
    );
    state.attach_consumer(Some(dir.path().join("db/consumer_keys.json")), Some(ledger.clone()));
    let pm = Arc::new(decentraai_agent_personal_memory::PersonalMemoryStore::new(dir.path().join("memory")));
    state.attach_personal_memory(pm);
    let api = serve_api(state, "127.0.0.1", 0).await.unwrap();
    let client = reqwest::Client::new();

    async fn create_key(client: &reqwest::Client, api: std::net::SocketAddr, master: &str, account: &str) -> String {
    ...
    async fn mcp(client: &reqwest::Client, api: std::net::SocketAddr, key: &str, name: &str, args: serde_json::Value) -> serde_json::Value {
    ...
    println!("Created consumer keys: A={}..., B={}..., C={}...", &ka[..12], &kb[..12], &kc[..12]);
    ...
    async fn advise(client: &reqwest::Client, backend: &str, prompt: &str) -> String {
        ...
        format!("advisory fallback for: {}", prompt.chars().take(80).collect::<String>())
    }

    // 1. Discover
    for (id,key) in [("agent-a",&ka),("agent-b",&kb),("agent-c",&kc)] {
        let r = mcp(&client, api, key, "discover_capabilities", serde_json::json!({})).await;
        println!("\n[{}] discover_capabilities → has hub={}, memory={}, society={}",
            id, r.to_string().contains("hub_publish_task"), r.to_string().contains("agent_memory"), r.to_string().contains("society_state"));
        let _ = advise(&client, &backend, &format!("Agent {id} discovered capabilities")).await;
    }
    // 2. Publish, 3. Discover, 4. Bid, 5. Negotiate, 6. Team, 7. Execute
    let task: serde_json::Value = serde_json::from_str(mcp(&client, api, &ka, "hub_publish_task", serde_json::json!({"title":"Translate","reward":300})).await["result"]["content"][0]["text"].as_str().unwrap()).unwrap();
    ...
    println!("[B] bid {}", mcp(&client, api, &kb, "hub_place_bid", serde_json::json!({"task_id":tid,"price":250})).await["result"]["content"][0]["text"].as_str().unwrap());
    println!("[C] bid {}", mcp(&client, api, &kc, "hub_place_bid", serde_json::json!({"task_id":tid,"price":200})).await["result"]["content"][0]["text"].as_str().unwrap());
    let prop: serde_json::Value = serde_json::from_str(mcp(&client, api, &ka, "hub_propose", serde_json::json!({"to":"agent-b","task_id":tid,"offer_price":150,"workshare":60})).await["result"]["content"][0]["text"].as_str().unwrap()).unwrap();
    let pid = prop["id"].as_str().unwrap();
    println!("[A→B] propose {pid} → {}", mcp(&client, api, &kb, "hub_decide_proposal", serde_json::json!({"proposal_id":pid,"accept":true})).await["result"]["content"][0]["text"].as_str().unwrap());
    println!("[A] form_team {}", mcp(&client, api, &ka, "hub_form_team", serde_json::json!({"task_id":tid,"members":[["agent-a",40],["agent-b",60]]})).await["result"]["content"][0]["text"].as_str().unwrap());
    let exec: serde_json::Value = serde_json::from_str(mcp(&client, api, &ka, "hub_execute", serde_json::json!({"task_id":tid})).await["result"]["content"][0]["text"].as_str().unwrap()).unwrap();
    println!("[A] execute → evidence {} settlement team {:?}", exec["evidence_id"].as_str().unwrap(), exec["team"]);
    ...
    for (key, acc) in [(&ka,"agent-a"),(&kb,"agent-b")] {
        ...
        println!("[{acc}] memory write → {}", r["result"]["content"][0]["text"].as_str().unwrap());
    }
    // isolation proof
    ...
    println!("\n[Isolation] A writes to B → {}", serde_json::to_string(&forbid).unwrap().chars().take(120).collect::<String>());

    let search = mcp(&client, api, &kb, "agent_memory_search", serde_json::json!({"agent_id":"agent-b","query":tid})).await;
    println!("[B] search memory for {tid} → {}", search["result"]["content"][0]["text"].as_str().unwrap().chars().take(150).collect::<String>());

    // Second task where history matters — Qwen advisory includes memory
    let task2: serde_json::Value = serde_json::from_str(mcp(&client, api, &kb, "hub_publish_task", serde_json::json!({"title":"Second","reward":500})).await["result"]["content"][0]["text"].as_str().unwrap()).unwrap();
    ...
    println!("\n[A] decision_hints for next task {} → {}", tid2, hints["result"]["content"][0]["text"].as_str().unwrap().chars().take(150).collect::<String>());
    let qwen_advice = advise(&client, &backend, &format!("Should agent-a bid on {tid2} given history with agent-b?")).await;
