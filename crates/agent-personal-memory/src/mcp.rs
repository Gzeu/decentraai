...
use crate::{
    PersonalMemoryStore, PersonalMemoryError,
...
pub fn extract_memory_read_request(raw: &str) -> Option<(String, Option<Vec<String>>)> {
    ...
    let categories = args.get("categories").and_then(|v| v.as_array()).map(|arr| {
...
pub fn extract_memory_search_request(raw: &str) -> Option<(String, String, Option<Vec<String>>, usize)> {
    let msg: Value = serde_json::from_str(raw).ok()?;
    if msg.get("method").and_then(|m| m.as_str()) != Some("tools/call") { return None; }
...
pub fn build_memory_read_response(
    ...
    let cats = categories.unwrap_or_else(|| vec![
        "identity".to_string(), "goals".to_string(), "capabilities".to_string(),
        "people".to_string(), "tasks".to_string(), "relationships".to_string(),
        "experiences".to_string(), "decisions".to_string(), "lessons".to_string(),
    ]);
...
pub fn build_memory_search_response(
...
pub async fn handle_tool_call(
    ...
    match name {
        "agent_memory_read" => {
            let agent_id = args.get("agent_id").and_then(|v| v.as_str()).ok_or(PersonalMemoryError::InvalidStructure("missing agent_id".to_string()))?;
            let categories = args.get("categories").and_then(|v| v.as_array()).map(|arr| {
                arr.iter().filter_map(|v| v.as_str().map(|s| s.to_string())).collect()
            });
        ...
        "agent_memory_write" => {
            let agent_id = args.get("agent_id").and_then(|v| v.as_str()).ok_or(PersonalMemoryError::InvalidStructure("missing agent_id".to_string()))?;
            let category = args.get("category").and_then(|v| v.as_str()).ok_or(PersonalMemoryError::InvalidStructure("missing category".to_string()))?;
            let entry = args.get("entry").cloned().ok_or(PersonalMemoryError::InvalidStructure("missing entry".to_string()))?;
            
            let agent_id_str = agent_id.to_string();
            let cat = category.to_string();
            
            store.write_entry(&agent_id_str, move |memory| {
                apply_write(memory, &cat, entry)
            }).await?;
            
            Ok(build_memory_write_response(true, None, format!("Updated {}", category)))
        }
        
        "agent_memory_search" => {
            let agent_id = args.get("agent_id").and_then(|v| v.as_str()).ok_or(PersonalMemoryError::InvalidStructure("missing agent_id".to_string()))?;
            let query = args.get("query").and_then(|v| v.as_str()).ok_or(PersonalMemoryError::InvalidStructure("missing query".to_string()))?;
            let categories = args.get("categories").and_then(|v| v.as_array()).map(|arr| {
                arr.iter().filter_map(|v| v.as_str().map(|s| s.to_string())).collect()
            });
            let limit = args.get("limit").and_then(|v| v.as_u64()).unwrap_or(10) as usize;
        ...
        _ => Err(PersonalMemoryError::InvalidStructure(format!("unknown tool: {}", name)))
    }
}

/// Apply a write to the personal memory
pub fn apply_write(memory: &mut crate::schema::PersonalMemory, category: &str, entry: Value) -> Result<(), PersonalMemoryError> {
    use crate::schema::*;
    use crate::now_ms;
    ...
    match category {
        ...
        "relationships" => {
            ...
            memory.relationships.relationships.insert(rel.agent_id.clone(), rel);
        ...
        "experiences" => {
            ...
            memory.experiences.experiences.sort_by_key(|b| std::cmp::Reverse(b.timestamp));
        ...
        "decisions" => {
            ...
            memory.decisions.decisions.sort_by_key(|b| std::cmp::Reverse(b.timestamp));
        ...
        "lessons" => {
            ...
            memory.lessons.lessons.sort_by_key(|b| std::cmp::Reverse(b.created_at));
            memory.lessons.frontmatter.updated_at = now_ms();
            memory.lessons.frontmatter.version += 1;
        }
        _ => return Err(PersonalMemoryError::InvalidStructure(format!("unknown category: {}", category))),
    }
    Ok(())
}
...
pub fn search_memory(
    ...
    let cats = categories.unwrap_or_else(|| vec![
        "identity".to_string(), "goals".to_string(), "capabilities".to_string(), "people".to_string(), "tasks".to_string(),
        "relationships".to_string(), "experiences".to_string(), "decisions".to_string(), "lessons".to_string()
    ]);
        ...
        match cat.as_str() {
            "identity" => {
                if memory.identity.name.to_lowercase().contains(&query_lower)
                    || memory.identity.description.to_lowercase().contains(&query_lower)
                    || memory.identity.persona.to_lowercase().contains(&query_lower) {
            ...
            "people" => {
                for (id, person) in &memory.people.people {
                    if person.display_name.as_ref().unwrap_or(id).to_lowercase().contains(&query_lower)
...
pub fn personal_memory_to_snapshot(memory: &crate::schema::PersonalMemory) -> PersonalMemorySnapshot {
    PersonalMemorySnapshot {
        agent_id: memory.identity.agent_id.clone(),
        identity: Some(memory.identity.description.clone()),
        goals: Some(memory.goals.goals.iter().map(|g| g.title.clone()).collect::<Vec<_>>().join(", ")),
        capabilities: Some(memory.capabilities.capabilities.iter().map(|c| c.name.clone()).collect::<Vec<_>>().join(", ")),
        people_count: memory.people.people.len(),
        recent_experiences: memory.experiences.experiences.iter().take(5).map(|e| ExperienceSummary {
            timestamp: e.timestamp,
            summary: e.summary.clone(),
            involved_agents: e.involved_agents.clone(),
            outcome: e.outcome.clone(),
        }).collect(),
        recent_decisions: memory.decisions.decisions.iter().take(5).map(|d| DecisionSummary {
            timestamp: d.timestamp,
            context: d.context.clone(),
            choice: d.choice.clone(),
            consequence: d.consequence.clone(),
        }).collect(),
        recent_lessons: memory.lessons.lessons.iter().take(5).map(|l| LessonSummary {
            timestamp: l.created_at,
            title: l.title.clone(),
            applies_to: l.applies_to.clone(),
        }).collect(),
        relationship_summaries: memory.people.people.iter().map(|(id, p)| {
            (id.clone(), format!("Trust: {:.2} | Interactions: {} | {}", p.trust_score, p.interaction_count, p.summary))
        }).collect(),
    }
}
