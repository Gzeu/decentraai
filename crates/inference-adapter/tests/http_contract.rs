...
async fn health_uses_backend_endpoint() {
    ...
    let health = server.mock_async(|when, then| { when.method(GET).path("/health"); then.status(200); }).await;
...
async fn maps_backend_http_error() {
    let server = MockServer::start_async().await;
    server.mock_async(|when, then| { when.method(GET).path("/health"); then.status(503).body("backend unavailable"); }).await;
    let backend = OpenAiCompatibleBackend::new(BackendConfig { base_url: server.base_url(), ..Default::default() }).unwrap();
...
async fn sends_bearer_key_without_logging_it() {
    let server = MockServer::start_async().await;
    let completion = server.mock_async(|when, then| { when.method(POST).path("/v1/chat/completions").header("authorization", "Bearer test-only-key"); then.status(200).json_body(serde_json::json!({"choices": [{"message": {"content": "ok"}}]})); }).await;
    let backend = OpenAiCompatibleBackend::new(BackendConfig { base_url: server.base_url(), api_key: Some("test-only-key".into()), ..Default::default() }).unwrap();
...
async fn request_timeout_is_configurable() {
    let server = MockServer::start_async().await;
    server.mock_async(|when, then| { when.method(GET).path("/health"); then.status(200).delay(Duration::from_millis(50)); }).await;
...
async fn stream_preserves_order_and_terminal_event() {
    ...
    let completion = server.mock_async(|when, then| { when.method(POST).path("/v1/chat/completions"); then.status(200).header("content-type", "text/event-stream").body(stream_body); }).await;
    let backend = OpenAiCompatibleBackend::new(BackendConfig { base_url: server.base_url(), ..Default::default() }).unwrap();
    let mut stream = backend.stream(request()).await.unwrap();
    let first = stream.next().await.unwrap().unwrap();
    let second = stream.next().await.unwrap().unwrap();
    let terminal = stream.next().await.unwrap().unwrap();
    assert_eq!((first.request_id.as_str(), first.sequence, first.text.as_str()), ("req-1", 0, "hello"));
    assert_eq!((second.request_id.as_str(), second.sequence, second.text.as_str()), ("req-1", 1, " world"));
    assert_eq!((terminal.sequence, terminal.text.as_str(), terminal.finish_reason.as_deref()), (2, "", Some("stop")));
